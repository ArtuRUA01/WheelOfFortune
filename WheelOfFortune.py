...
class WOFComputerPlayer(WOFPlayer):
    ...
    def getPossibleLetters(self, guessed):
        possible_letters = []
        unpossible_letters = []
        unpossible_letters.extend(guessed)
        if self.prizeMoney < VOWEL_COST:
            unpossible_letters.extend(VOWELS)
        for i in LETTERS:
            if i in unpossible_letters:
                continue
            possible_letters.extend(i)
    ...
    def getMove(self, category, obscuredPhrase, guessed):
        ...
        if possible_letters == '':
            ...
            if self.smartCoinFlip() == True:
                ...
                for i in random.shuffle(self.SORTED_FREQUENCIES):
                    for j in possible_letters:
                        if i == j:
                            return i
